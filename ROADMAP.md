...
- [ ] **Dead `max_trailing_dd_pct` config value.**
...
- [ ] **Position sizing mixes `point` and `tick_size`.**
...
- [ ] **`requirements.txt` is mixed-encoding (broken file).**
...
- [ ] **No crash isolation in the main loop.**
...
- [ ] **`mt5.order_send()` result never null-checked.**
...
- [ ] **Emergency close has no verification or retry.**
...
- [ ] `api/server.py` has **no authentication** on any endpoint, including `POST /api/config` (lets anyone rewrite your risk limits, strategy, or symbols).
  **Fix:** add a simple API key header check (`X-API-Key`) validated against an env var, minimum viable for personal use.
- [ ] `CORSMiddleware(allow_origins=["*"], allow_credentials=True, ...)` — wide open.
  **Fix:** restrict to `http://localhost:3000` (or wherever your frontend actually runs).
- [ ] `start_dev.bat` / `start_osiris.bat` launch uvicorn with `--host 0.0.0.0`, exposing it to your whole LAN (and to the internet if you ever port-forward or run this on a VPS/cloud box).
...
- [ ] No secrets management pattern established yet (fine today since nothing sensitive is hardcoded — keep it that way). Add a `.env` + `python-dotenv` convention now, before you add anything like a Telegram/Discord alert token or a cloud API key.

---

## 3. Repository Hygiene

- [ ] `.gitignore` rules (`*.ex5`, `*.log`, `logs/`, `*.dat`) are correct now but don't apply retroactively — 133 `.ex5` files, `logs/20260916.log`, and `experts.dat` are still tracked.
  **Fix:**
  ```bash
  git rm -r --cached "**/*.ex5" logs/ experts.dat
  git commit -m "Untrack compiled binaries and runtime logs"
  ```
- [ ] Delete the stray junk file `PropFirm_System/nul\`n\`necho` (debris from a broken shell command; duplicate of `start_dev.bat` content).
- [ ] Remove hardcoded personal path (`d:/Omnity Era/Antigravity/MQL5/...`) from `DOCUMENTATION.md` — use relative paths so the docs aren't tied to your machine.
- [ ] Reconcile the three overlapping docs: top-level `DOCUMENTATION.md` (says 5%/10% limits, FinRL default), `PropFirm_System/README.md`, and `PropFirm_System/PROJECT_DOCUMENTATION.md`. Pick **one** source of truth and have the others link to it, or delete the stale ones. Right now they contradict your actual `config.json` (1%/3%, `NYTrendContinuation`).
- [ ] `PropFirm_System/.gitignore` covers `node_modules/`, `__pycache__/`, `.env`, `state.json`, `bot.log` — good. Add `*.log`, `strategy/models/*.zip` (trained models are binary artifacts; consider a separate `models/` storage or Git LFS if you want them versioned) once you're training more than a handful.
- [ ] Add a top-level `LICENSE` file (even a personal "all rights reserved" is better than none) and consolidate to a single root `README.md` that GitHub will render on the repo landing page.
...
- [ ] Fix all 6 items in Section 1.
- [ ] Fix all items in Section 2 (security).
- [ ] Fix all items in Section 3 (hygiene).
- [ ] Add a `CHANGELOG.md` and start tagging versions (`v0.1.0`, etc.) so you can tell which commit a given trading session ran on.
...
### Stage 2 — Correctness Validation (prove the risk math is right)
- [ ] Add `pytest` + a `tests/` folder. Priority order:
  - `test_position_sizing.py` — mock `symbol_info` with `point != tick_size`, assert lot size math.
  - `test_daily_reset.py` — assert `start_of_day_balance` resets at the **broker's** day boundary, not the host machine's local midnight (see Stage 2 note below).
  - `test_risk_limits.py` — assert `check_risk_status()` actually reflects `config.json` changes end-to-end (this would have caught the trailing-DD bug).
- [ ] **Fix the timezone/day-boundary issue directly**: `schedule.every().day.at("00:00")` and `datetime.now()` both use the *host machine's* local clock. Prop firms define the daily reset in **broker/server time**, which is rarely your local timezone. Derive the day boundary from MT5 server time (e.g., from the latest tick's timestamp) instead of `datetime.now()`.
- [ ] Run `PropFirmBacktester` against your actual `NYTrendContinuation` strategy (the doc only ever validated the older EMA strategy) across at least 2 years of data, multiple symbols, and explicitly simulate spread + a few points of slippage on every fill — not just SL/TP touching the candle range.
- [ ] Walk-forward validate, don't just single-split: train/tune on year 1, test untouched on year 2, repeat rolling. If you're still using the FinRL/PPO strategy at all, this matters even more — a single 80/20 split on 20–50k M15 candles overfits easily.

**Exit gate:** you can explain, from memory, exactly when the daily-loss clock resets in your broker's timezone and prove it with a passing test.

### Stage 3 — Robustness & Observability (survive being unattended)
- [ ] Per-cycle exception isolation (Section 1, item 4) is done and tested by deliberately raising an exception mid-loop to confirm the bot keeps running.
- [ ] Add reconnect logic: if `mt5.initialize()` / terminal IPC drops mid-session, detect it and re-attach rather than crash.
- [ ] Add basic alerting — a Telegram or Discord webhook is enough for personal use — for: daily loss limit hit, trailing DD hit, emergency close triggered, bot crashed/restarted, order rejected repeatedly.
- [ ] Structured logging: move from plain `logging.info(f"...")` strings toward consistent fields (symbol, action, lot, retcode) so you can grep/aggregate later.
- [ ] Process supervision: run `main.py` under something that auto-restarts it on crash (NSSM/Task Scheduler on Windows, or a simple watchdog script) — but only *after* Stage 3's crash-isolation work, so auto-restart is a safety net, not a crutch masking the real bug.
...
- [ ] Replace the fixed "max daily trades: 999 (temporarily increased for testing)" with a real configurable cap once you've decided what limit you actually want live.
